...
class GameController extends AbstractController
{
    ...
    #[Route('/game/{id}', name: 'get_game_info', methods:['GET'])]
    ...
    public function inviteToGame(Request $request, EntityManagerInterface $entityManager, $id, $playerRightId): JsonResponse
    {
        ...
        if (empty($currentUserId) || ctype_digit($currentUserId) === false) {
            return new JsonResponse('User not found', 401);
        }
        ...
        if (ctype_digit($playerRightId) === false) {
            return new JsonResponse('Game not found', 404);
        }
    ...
    #[Route('/game/{identifiant}', name: 'game_play', methods:['PATCH'])]
    public function gamePlay(Request $request, EntityManagerInterface $entityManager, $identifiant): JsonResponse
    {
        $currentUserId = $request->headers->get('X-User-Id');
        ...
        if ($userIsPlayerLeft === false && $userIsPlayerRight === false) {
        ...
        if ($game->getPlayLeft() !== null && $game->getPlayRight() !== null) {
            $game->setResult($this->resolveGameMovesResult(
                $game->getPlayLeft(),
                $game->getPlayRight()
            ));
    ...
    private function resolveGameMovesResult(string $leftChoice, string $rightChoice): string
    {
        ...
        switch ($leftChoice) {
            case 'rock':
                return ($rightChoice === 'scissors') ? 'winLeft' : 'winRight';
            case 'paper':
                return ($rightChoice === 'rock') ? 'winLeft' : 'winRight';
            case 'scissors':
                return ($rightChoice === 'paper') ? 'winLeft' : 'winRight';
        }
